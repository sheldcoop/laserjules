...
# --- SIDEBAR NAVIGATION (CORRECTED ORDER) ---
with st.sidebar:
    st.header("App Mode")
...
# --- MASTER MODE: PROCESS RECOMMENDER (UPDATED & FIXED) ---
if st.session_state.app_mode == "Process Recommender":
    st.header("Generate a Process Recipe")
    st.markdown("---")
    st.info("Input your goal and material properties to generate a recommended starting recipe.")
    
    col1, col2, col3 = st.columns(3)
    with col1:
        st.subheader("🎯 Your Goal")
        target_via_um = st.number_input("Target Via Diameter (µm)", min_value=1.0, value=12.0)
        material_thickness_um = st.number_input("Material Thickness (µm)", min_value=1.0, value=25.0)
        
    with col2:
        st.subheader("🔬 Material Properties")
        st.caption("_(Use the 'Material Analyzer' to find these values)_")
        optimal_fluence = st.number_input("Optimal Fluence (J/cm²)", min_value=0.1, value=2.0, format="%.3f")
        ablation_rate = st.number_input("Ablation Rate at Optimal Fluence (µm/pulse)", min_value=0.01, value=0.9, format="%.3f")

    with col3:
        st.subheader("⚙️ Machine Settings")
        beam_spot_um = st.number_input("Beam Spot Size (µm)", min_value=1.0, value=15.0)
        rep_rate_khz = st.number_input("Repetition Rate (kHz)", min_value=1.0, value=50.0)
    ...
    if st.button("Generate Recipe", type="primary"):
        ...
        # Correctly calculate the required average power
        required_power_mW = required_energy_uJ * rep_rate_khz
        
        # Save the complete recipe to session state
        st.session_state.recipe = {
            ...
            "Required Pulse Energy (µJ)": f"{required_energy_uJ:.3f}",
            "Required Average Power (mW)": f"{required_power_mW:.1f}",
            "Required Number of Shots": int(required_shots)
        }

    # Display the complete recipe card if it exists
    if st.session_state.recipe:
        st.markdown("---")
        st.markdown(f'<p class="results-header">Recommended Process Recipe</p>', unsafe_allow_html=True)
        
        recipe = st.session_state.recipe
        col1, col2, col3 = st.columns(3)
        col1.metric("Required Pulse Energy", f"{recipe['Required Pulse Energy (µJ)']} µJ")
        col2.metric("Required Average Power", f"{recipe['Required Average Power (mW)']} mW")
...
# --- MATERIAL ANALYZER ---
    ...
    st.markdown("---")
    st.info("Upload or paste your experimental data to characterize a new material.")
    
    number_of_shots = st.number_input("Number of Shots (used in experiment)", min_value=1, value=50)
    
    input_method = st.radio("Select Data Input Method", ["Paste Data", "Upload CSV"], horizontal=True)
    
    if input_method == "Paste Data":
        col1, col2 = st.columns(2)
        fluence_str = col1.text_area("Paste Fluence Data (J/cm²)", placeholder="1.5\n2.0\n2.5\n3.0")
        depth_str = col2.text_area("Paste Measured Depth Data (µm)", placeholder="15\n45\n70\n90")
    else:
        uploaded_file = st.file_uploader("Upload CSV file", type="csv")
        st.markdown("Your CSV should have two columns: 'Fluence' and 'Depth'")
...
# --- MASK FINDER ---
    ...
    with st.expander("Show Formula"):
        st.latex(r'''\text{Mask Size (mm)} = \frac{\text{Target Via Diameter (µm)} \times \text{Demagnification Factor}}{1000}''')
    if demag_factor > 0:
...
# --- UNIVERSAL RESULTS DISPLAY (for calculators) ---
if st.session_state.results_df is not None:
    st.markdown("---")
    st.markdown(f'<p class="results-header">Calculation Results</p>', unsafe_allow_html=True)
    cols_to_show = list(st.session_state.results_df.columns) # Dynamically show all columns
    st.dataframe(st.session_state.results_df[cols_to_show].style.format(precision=3), use_container_width=True, hide_index=True)

# --- ANALYSIS RESULTS DISPLAY ---
if st.session_state.analysis_results is not None:
    results = st.session_state.analysis_results
    st.markdown("---")
    st.markdown(f'<p class="results-header">Material Analysis Results</p>', unsafe_allow_html=True)
    ...
    fig.update_traces(marker=dict(size=10, color='#ef4444')); fig.update_layout(xaxis_title="Fluence (J/cm²) [Log Scale]", yaxis_title="Ablation Rate (µm/pulse)")
